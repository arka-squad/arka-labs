...
export const GET = withAuth(['viewer', 'editor', 'admin', 'owner'], async (req: NextRequest, { params }: { params: { id: string } }) => {
  ...
  const res = NextResponse.json({ job, result: results.get(params.id) || null });
  res.headers.set(TRACE_HEADER, traceId);
  return res;
});
