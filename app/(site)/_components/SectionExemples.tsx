    ...
    <section aria-labelledby="exemples" className="py-16">
      <div className="mx-auto max-w-[1440px] px-6">
        <header className="text-center max-w-3xl mx-auto">
          <h2 id="exemples" className="text-3xl md:text-4xl font-semibold" style={{ color: 'var(--site-text)' }}>Exemples</h2>
          <p className="mt-2" style={{ color: 'var(--site-muted)' }}>Trois cas concrets pour démarrer</p>
        </header>
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          {items.map((c, i) => (
            <article key={i} className={`relative overflow-hidden rounded-[20px] bg-white ring-1 ring-black/5 shadow-[0_12px_30px_rgba(15,23,42,.08)] p-6 ${['cardGlowA','cardGlowB','cardGlowC'][i%3]}`}>
              ...
              <p className="mt-2 text-sm leading-relaxed" style={{ color: 'var(--site-muted)' }}>{c.d}</p>
            </article>
          ))}
        </div>
      </div>
