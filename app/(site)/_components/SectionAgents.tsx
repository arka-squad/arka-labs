    ...
    <section aria-labelledby="agents" className="py-16">
      <div className="mx-auto max-w-[1440px] px-6">
        <header className="text-center max-w-3xl mx-auto">
          <h2 id="agents" className="text-3xl md:text-4xl font-semibold" style={{ color: 'var(--site-text)' }}>Nos agents experts</h2>
          <p className="mt-2" style={{ color: 'var(--site-muted)' }}>Une squad spécialisée qui travaille ensemble</p>
        </header>
          ...
          <div className="min-w-[720px] grid grid-cols-4 gap-6">
            {agents.map((a, i) => (
              <article key={a.name} className="rounded-[20px] bg-white ring-1 ring-black/5 shadow-[0_12px_30px_rgba(15,23,42,.08)] p-6">
                ...
                <h3 className="text-base font-semibold" style={{ color: 'var(--site-text)' }}>{a.name}</h3>
                <p className="mt-1 text-sm" style={{ color: 'var(--site-muted)' }}>{a.desc}</p>
              </article>
            ))}
          </div>
        </div>
