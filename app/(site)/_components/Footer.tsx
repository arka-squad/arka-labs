...
export default function Footer({ links = DEFAULT_LINKS, showLogo = true, year = new Date().getFullYear(), onLinkClick }: FooterProps) {
  return (
    <footer role="contentinfo" className="mt-24 border-t" style={{ borderColor: 'rgba(0,0,0,.10)' }}>
      <div className="mx-auto max-w-[90rem] px-6 py-10 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm" style={{ color: '#475569' }}>
              ...
              <>
                <li key={l.href}>
                  <a
                    ...
                    className="hover:underline underline-offset-4 hover:text-[#0F172A] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20 rounded"
                  >
                    {l.label}
                  </a>
                </li>
