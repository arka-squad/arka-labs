...
const DEFAULT_ITEMS: AudienceItem[] = [
  { id: 'rh', icon: <Users className="w-5 h-5" aria-hidden />, title: 'RH solo et petites Ã©quipes', points: ['DÃ©charger le quotidien', 'Sans embaucher'], glow: 'A' },
  { id: 'pme', icon: <Briefcase className="w-5 h-5" aria-hidden />, title: 'PME et directions', points: ['Piloter missions & preuves', 'DÃ©cider en confiance'], glow: 'B' },
  { id: 'ecoles', icon: <GraduationCap className="w-5 h-5" aria-hidden />, title: 'Ã‰coles et formations', points: ['ScÃ©narios guidÃ©s', '10Ã— plus vite', 'Sans donnÃ©es sensibles'], glow: 'C' },
  { id: 'extensions', icon: <CheckCircle className="w-5 h-5" aria-hidden />, title: 'Extensions', long: 'Compta, Finance, Marketing, Support. La squad sâ€™installe, documente et tient la durÃ©e.', glow: 'D' },
];

export default function Audience({ title = 'Pour qui', subtitle = 'Aujourdâ€™hui et demain', image = { src: '/assets/hero/arkabox-board.avif', alt: 'Publics visÃ©s par Arka' }, items = DEFAULT_ITEMS }: AudienceProps) {
  return (
